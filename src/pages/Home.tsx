          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <h1 className="text-5xl lg:text-7xl font-bold tracking-tight text-slate-900 leading-[1.1] mb-6">
                Future-Proof Your <span className="text-red-600">Business</span> with Expert IT.
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  to="/contact"
                  className="inline-flex items-center justify-center px-8 py-4 rounded-full bg-red-600 text-white font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-200 active:scale-95 group"
                >
                  Get Started Today
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="grid grid-cols-2 gap-6">
                {industries.map((industry, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700">
                    <industry.icon className="w-6 h-6 text-red-400" />
            ...
            <div className="grid gap-6">
              {caseStudies.map((study, i) => (
                <div key={i} className="p-8 rounded-3xl bg-red-600 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:scale-110 transition-transform">
                    <BarChart3 className="w-24 h-24" />
                  </div>
                  <div className="relative z-10">
                    <span className="text-xs font-bold uppercase tracking-widest text-red-200 mb-2 block">Case Study</span>
                    <h3 className="text-2xl font-bold mb-3">{study.title}</h3>
                    <p className="text-red-100 text-sm mb-6">{study.description}</p>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {testimonials.map((t, i) => (
              <div key={i} className="p-10 rounded-3xl bg-slate-50 border border-slate-100 relative">
                <Quote className="absolute top-8 right-8 w-12 h-12 text-red-100" />
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-600 rounded-[3rem] p-12 lg:p-20 text-center text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row justify-center gap-4">
              <Link
                to="/contact"
                className="px-10 py-4 bg-white text-red-600 font-bold rounded-full hover:bg-red-50 transition-all shadow-xl active:scale-95"
              >
                Contact Us Now
              </Link>
              <a
                href={`tel:${COMPANY_DETAILS.phone}`}
                className="px-10 py-4 bg-red-500 text-white font-bold rounded-full border border-red-400 hover:bg-red-400 transition-all active:scale-95"
              >
