        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <motion.h1
              ...
              className="text-5xl md:text-7xl font-bold tracking-tight text-brand-primary mb-8 leading-[1.1]"
            >
              Empowering British Business Through <span className="text-brand-accent italic font-serif">Intelligent IT</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.2 }}
              className="text-xl text-brand-primary/80 mb-12 leading-relaxed max-w-2xl mx-auto"
            >
            ...
            >
              <Link
                to="/services"
                className="w-full sm:w-auto px-8 py-4 bg-brand-accent text-white rounded-full font-semibold hover:bg-brand-accent-hover transition-all shadow-lg shadow-brand-accent/20 flex items-center justify-center gap-2 group"
              >
                Explore Services
              ...
              <Link
                to="/contact"
                className="w-full sm:w-auto px-8 py-4 bg-white text-brand-primary border border-brand-primary/20 rounded-full font-semibold hover:bg-brand-surface transition-all flex items-center justify-center"
              >
                Book a Consultation
              </Link>
            </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <motion.div
                ...
                className="bg-white p-8 rounded-3xl border border-brand-primary/10 shadow-sm"
              >
                <div className="mb-6">{benefit.icon}</div>
                <h3 className="text-xl font-bold text-brand-primary mb-4">{benefit.title}</h3>
                <p className="text-brand-primary/80 leading-relaxed">{benefit.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-sm font-bold text-brand-accent uppercase tracking-widest mb-4">Our Expertise</h2>
              <p className="text-4xl font-bold text-brand-primary leading-tight">
                Comprehensive IT Solutions Tailored for the Modern Enterprise
              </p>
            </div>
            <Link to="/services" className="text-brand-accent font-semibold flex items-center gap-2 hover:gap-3 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.slice(0, 3).map((service, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="group p-8 rounded-3xl border border-brand-primary/10 hover:border-brand-accent/30 transition-all hover:shadow-xl hover:shadow-brand-accent/10"
              >
                <h3 className="text-2xl font-bold text-brand-primary mb-4">{service.title}</h3>
                <p className="text-brand-primary/80 mb-8 line-clamp-2">{service.description}</p>
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-brand-primary font-bold group-hover:text-brand-accent transition-colors"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <div className="text-4xl font-bold text-brand-accent mb-2">£45k</div>
                  <div className="text-sm text-white/60 uppercase tracking-wider">Annual Savings</div>
                </div>
                <div>
                  <div className="text-4xl font-bold text-brand-accent mb-2">99.9%</div>
                  <div className="text-sm text-white/60 uppercase tracking-wider">Uptime Achieved</div>
                </div>
              </div>
            </div>
            <div className="relative">
              ...
              <div className="absolute -bottom-8 -left-8 bg-brand-accent p-8 rounded-3xl shadow-2xl max-w-xs">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-white/70">Managing Partner, Sterling & Co.</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-sm font-bold text-brand-accent uppercase tracking-widest mb-4">Client Voices</h2>
            <p className="text-4xl font-bold text-brand-primary">What Our Partners Say</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              {
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="bg-brand-surface p-8 rounded-3xl border border-brand-primary/10"
              >
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Zap key={i} className="w-4 h-4 fill-brand-accent text-brand-accent" />
                  ))}
                </div>
                <p className="text-brand-primary/80 mb-8 leading-relaxed italic">"{t.text}"</p>
                <div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-brand-accent rounded-[3rem] p-12 md:p-20 text-center relative overflow-hidden">
            ...
            <p className="text-white/90 text-xl mb-12 max-w-2xl mx-auto relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 relative z-10">
              <Link
                to="/contact"
                className="w-full sm:w-auto px-10 py-5 bg-white text-brand-accent rounded-full font-bold text-lg hover:bg-brand-surface transition-all shadow-xl"
              >
                Get Started Today
              </Link>
              <Link
                to="/pricing"
                className="w-full sm:w-auto px-10 py-5 bg-brand-primary text-white rounded-full font-bold text-lg hover:bg-brand-accent transition-all"
              >
                View Pricing
              </Link>
            </div>
