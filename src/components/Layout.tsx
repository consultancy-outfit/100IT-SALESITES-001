...
import FradelLodgeLogo from "../assets/Fradel Lodge.svg";
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900 font-sans selection:bg-violet-100 selection:text-violet-900">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <Link to="/" className="flex items-center group">
              <div className="w-[250px] h-[50px] overflow-hidden flex items-center cursor-pointer">
                <img
                  src={FradelLodgeLogo}
              ...
              <div className="px-4 py-6 space-y-4">
                {NAV_LINKS.map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                    className="block text-lg font-medium text-stone-900 hover:text-violet-600"
                  >
                    {link.name}
                  </Link>
                ))}
                <Link
                  to="/contact"
                  className="block w-full text-center bg-violet-600 text-white py-3 rounded-xl font-semibold"
                >
                  Get Started
                </Link>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="space-y-6">
              ...
              <p className="text-sm leading-relaxed">
                Empowering UK businesses with cutting-edge IT solutions, robust
                cybersecurity, and strategic technology consultancy.
              </p>
              <div className="flex space-x-4">
                {/* Social placeholders */}
                <div className="w-8 h-8 rounded-full bg-stone-800 flex items-center justify-center hover:bg-violet-600 transition-colors cursor-pointer">
